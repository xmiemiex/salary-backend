...
assert.match(page, /monthly\/refresh|post\('refresh'/);
assert.match(page, /本月手续费/);
assert.match(page, /保存本月费率/);
assert.match(page, /保存花费/);
assert.match(page, /currentMonth.current === submittedMonth/);
assert.match(page, /request === sequence.current/);
assert.match(page, /合计/);
assert.doesNotMatch(page, /dangerouslySetInnerHTML|rawPayload|passwordHash|tokenHash/);
console.log('monthly finance page structure and existing formatting tests passed');
