...
type Data = { localSample: boolean; month: string; locked: boolean; columns: { key: string; name: string }[]; rates: Record<string, string | null>; rows: Row[]; totals: Amounts; sources: { key: string; name: string; status: string; reason: string | null; lastSuccessAt: string | null; coveredThrough: string | null; updatedAt: string | null }[]; complete: boolean; refreshing: boolean; batchId: string | null; queriedAt: string; coveredThrough: string | null };
...
export function DashboardPage({ actor, onNavigate }: { actor: Actor; onNavigate: (path: string) => void }) {
  ...
  return <section className="monthly-finance">
    ...
    <div className="finance-status"><Space wrap>{data?.localSample && <Tag color="blue">本地模拟样例</Tag>}<Tag color={data?.complete ? 'green' : 'orange'}>{data?.refreshing ? '正在刷新数据' : data?.complete ? '数据已同步' : '数据尚不完整'}</Tag>{data?.locked && <Tag>已锁账</Tag>}<span>查询时间 {formatDashboardTime(data?.queriedAt ?? null)} · 共同覆盖至 {data?.coveredThrough ? formatDashboardTime(data.coveredThrough) : '尚未完整覆盖'} · GMT+8</span><Button type="link" size="small" onClick={() => onNavigate('/data-sync')}>同步详情</Button></Space></div>
