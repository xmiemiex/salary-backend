import { ManualIncomeEditor } from './ManualIncomeEditor';
import { CakeMonthlyReviewPanel, type CakeMonthlyReview } from './CakeMonthlyReviewPanel';
...
type Data = { cakeReviews: CakeMonthlyReview[]; coverageScope: 'full_month' | 'month_to_date' | 'future'; localSample: boolean; month: string; locked: boolean; columns: { key: string; name: string }[]; rates: Record<string, string | null>; rows: Row[]; totals: Amounts; sources: { key: string; name: string; status: string; statusLabel?: string; unmatchedCount?: number; reason: string | null; lastSuccessAt: string | null; coveredThrough: string | null; updatedAt: string | null }[]; complete: boolean; refreshing: boolean; batchId: string | null; queriedAt: string; coveredThrough: string | null };
...
export function DashboardPage({ actor, onNavigate }: { actor: Actor; onNavigate: (path: string) => void }) {
  ...
  const [panel, setPanel] = useState<'fees' | 'detail' | 'adpos' | 'sub' | 'manual-income' | null>(null);
  ...
  const editManualIncome = (r: Row) => { setRow(r); setAmount(r.otherIncome); setPanel('manual-income'); };
  ...
  const columns: ColumnsType<Row> = [
    ...
    { title: '联盟收入明细 · USD', className: 'finance-income-head', children: [...(data?.columns ?? []).map(c => numberColumn(c.name, r => r.byAffiliate[c.key] ?? '0', 145, c.key)), { title: '其他手动收入', width: 210, align: 'right', render: (_, r) => <Space><button className="finance-money" onClick={() => detail(r)}>{money(r.otherIncome)}</button>{actor.permissions.includes('income.import') && <Button size="small" aria-label={`编辑 ${r.subId} 其他手动收入`} disabled={data?.locked || r.key === 'unassigned' || r.attributionPending} onClick={() => editManualIncome(r)}>编辑</Button>}</Space> }] },
  ...
  return <section className="monthly-finance">
    ...
    {data && <><div className="finance-source-line">{data.sources.map(s => <Space key={s.key} size={4}><span>{s.name}</span><Tag color={s.status === 'completed' ? 'green' : ['failed','partial'].includes(s.status) ? 'red' : 'default'}>{s.statusLabel ?? statuses[s.status] ?? s.status}</Tag>{s.reason && <span>{s.reason}</span>}{!!s.unmatchedCount && canNavigateDashboardTarget(actor, '/sync-unmatched-events') && <Button type="link" size="small" onClick={() => onNavigate('/sync-unmatched-events')}>未匹配事件</Button>}{s.reason && canNavigateDashboardTarget(actor, '/data-sync') && <Button type="link" size="small" onClick={() => onNavigate('/data-sync')}>同步详情</Button>}{s.lastSuccessAt && <span>上次成功 {formatDashboardTime(s.lastSuccessAt)} · 覆盖至 {formatDashboardTime(s.coveredThrough)}</span>}{canRefresh && ['failed','partial'].includes(s.status) && <Button type="link" size="small" disabled={data.locked || data.refreshing || saving} onClick={() => void post('refresh', { source: s.key })}>重试</Button>}</Space>)}</div>
    <CakeMonthlyReviewPanel reviews={data.cakeReviews ?? []} month={month} locked={data.locked} actor={actor} onChanged={load} onNavigate={onNavigate} />
    ...
    <Drawer title={panel === 'fees' ? `${month} · 本月手续费` : panel === 'adpos' ? `${row?.subId} · Adpos 原始花费` : panel === 'manual-income' ? `${row?.subId} · 其他手动收入` : panel === 'sub' ? '统一 SUB ID' : `${row?.subId ?? ''} · 来源明细`} open={panel !== null} onClose={() => !saving && setPanel(null)} width="min(520px, 100vw)" destroyOnClose>
      {error && <Alert type="error" message={error} showIcon style={{ marginBottom: 16 }} />}
      {panel === 'manual-income' && row && <ManualIncomeEditor key={`${month}:${row.key}`} month={month} rowKey={row.key} subId={row.subId} locked={!!data?.locked} onSaved={load} onBusy={setSaving} />}
      {panel === 'detail' && row && actor.permissions.includes('income.import') && <Button disabled={data?.locked || row.key === 'unassigned' || row.attributionPending} onClick={() => editManualIncome(row)}>编辑其他手动收入</Button>}
