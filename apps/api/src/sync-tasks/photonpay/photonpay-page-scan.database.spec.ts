...
dbDescribe('PhotonPay durable page state on isolated PostgreSQL', () => {
  ...
  it('rejects late success and failure from an older attempt in the same executor instance', async () => {
    ...
    const current = { ...context, durablePageScan: { leaseOwner: internal.instanceId, attemptCount: 2 } };
    const scan = new PhotonPayPageScan(db as never, current, { credential: 'atomic-test' });
    await scan.save(state);
    const completion = { ...result, completedPageScanFingerprint: scan.fingerprint };
    audit.success.mockRejectedValueOnce(new Error('simulated transaction interruption'));
    await expect(internal.finishSuccess(context.taskId, { ...claim, attemptCount: 2 }, completion)).rejects.toThrow('simulated');
    expect((await db.syncTask.findUniqueOrThrow({ where: { id: context.taskId } })).status).toBe('running');
    expect(await scan.load()).toEqual(state);
    audit.success.mockClear();
    await db.syncTask.update({ where: { id: context.taskId }, data: { leaseExpiresAt: new Date(0) } });
    await internal.finishSuccess(context.taskId, { ...claim, attemptCount: 2 }, completion);
    await internal.finishFailure(context.taskId, { ...claim, attemptCount: 2 }, 'BUSINESS_REJECTED', 'expired');
    expect((await db.syncTask.findUniqueOrThrow({ where: { id: context.taskId } })).status).toBe('running');
    expect(await scan.load()).toEqual(state);
    await db.syncTask.update({ where: { id: context.taskId }, data: { leaseExpiresAt: new Date(Date.now() + 60000) } });
    await internal.finishSuccess(context.taskId, { ...claim, attemptCount: 2 }, completion);
