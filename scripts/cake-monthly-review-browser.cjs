...
const output = path.join(root, 'tmp/cake-monthly-review');
...
(async () => {
  ...
  try {
    ...
    // CAKE review mutations use real API authentication and isolated PostgreSQL.
    const cake = accounts.find(a => a.platform === 'cake');
    const cakeRow = page.locator(`[data-cake-review="${cake.id}"]`);
    await expect(page.locator('[data-cake-review]')).toHaveCount(1);
    await expect(cakeRow.getByText('未核对GMT+8时区佣金', { exact: true })).toBeVisible();
    const incomeBeforeReview = await db.incomeRecord.count();
    const confirmReview = async () => {
      await cakeRow.getByRole('button', { name: '已核对，无需调整', exact: true }).click();
      await page.locator('.ant-modal-confirm').getByRole('button', { name: '已核对，无需调整', exact: true }).click();
      await expect(page.locator('.ant-modal-confirm')).toHaveCount(0);
    };
    await confirmReview();
    await expect(cakeRow.getByText('已核对GMT+8时区佣金，无需调整', { exact: true })).toBeVisible();
    expect(await db.incomeRecord.count()).toBe(incomeBeforeReview);
    const confirmation = await db.cakeMonthlyIncomeReview.findFirstOrThrow();
    expect(confirmation.confirmedBy).toBe(user.id);
    await clickRefresh();
    await expect(page.getByText('刷新失败', { exact: true }).first()).toBeVisible();
    await expect(cakeRow.getByText('已核对GMT+8时区佣金，无需调整', { exact: true })).toBeVisible();
    await page.screenshot({ path: path.join(output, 'cake-confirmed.png'), fullPage: true });
    await cakeRow.getByRole('button', { name: '取消核对', exact: true }).click();
    await page.locator('.ant-modal-confirm').getByRole('button', { name: '取消核对', exact: true }).click();
    await expect(cakeRow.getByText('未核对GMT+8时区佣金', { exact: true })).toBeVisible();
    await confirmReview();
    const native = await db.incomeRecord.findFirstOrThrow({ where: { affiliateAccountId: cake.id, settlementMonth: month } });
    await db.incomeRecord.update({ where: { id: native.id }, data: { incomeUsd: native.incomeUsd.plus(1) } });
    await page.reload(); await expect(picker).toBeVisible(); await picker.fill('2026-09'); await picker.press('Enter'); await picker.press('Tab');
    await expect(cakeRow.getByText('待复核GMT+8时区佣金', { exact: true })).toBeVisible();
    await page.screenshot({ path: path.join(output, 'cake-needs-review.png'), fullPage: true });
    await cakeRow.getByRole('button', { name: '取消核对', exact: true }).click();
    await page.locator('.ant-modal-confirm').getByRole('button', { name: '取消核对', exact: true }).click();
    await expect(cakeRow.getByText('未核对GMT+8时区佣金', { exact: true })).toBeVisible();
    const adjustedBase = native.incomeUsd.plus(1);
    const adjustment = await db.incomeRecord.create({ data: { employeeId: native.employeeId, affiliateAccountId: cake.id, settlementMonth: month, source: 'cake_adjustment', subField: native.subField, subValue: native.subValue, incomeUsd: '5', status: 'draft', rawData: { kind: 'cake_sub_revenue_adjustment', baseRevenueUsd: adjustedBase.toString(), targetRevenueUsd: adjustedBase.plus(5).toString(), adjustmentUsd: '5', reason: 'SIMULATED_LOCAL_ONLY', stale: false } } });
    const reloadSeptember = async () => { await page.reload(); await expect(picker).toBeVisible(); await picker.fill('2026-09'); await picker.press('Enter'); await picker.press('Tab'); };
    await reloadSeptember();
    await expect(cakeRow.getByText('未核对GMT+8时区佣金', { exact: true })).toBeVisible();
    await db.incomeRecord.update({ where: { id: adjustment.id }, data: { status: 'confirmed' } });
    await reloadSeptember();
    await expect(cakeRow.getByText('已有已确认调整（1 条）', { exact: true })).toBeVisible();
    await page.screenshot({ path: path.join(output, 'cake-adjusted.png'), fullPage: true });
    await db.incomeRecord.update({ where: { id: adjustment.id }, data: { status: 'disabled' } });
    await reloadSeptember();
    await expect(cakeRow.getByText('未核对GMT+8时区佣金', { exact: true })).toBeVisible();
    await cakeRow.getByRole('button', { name: '查看 / 调整', exact: true }).click();
    await expect(page.getByText('CAKE SUB 月度收入调整', { exact: true })).toBeVisible();
    expect(new URL(page.url()).searchParams.get('affiliateAccountId')).toBe(cake.id);
    expect(new URL(page.url()).searchParams.get('settlementMonth')).toBe('2026-09');
    await expect(page.getByText('original-1-2', { exact: true }).first()).toBeVisible();
    await page.goto('http://localhost:5190/dashboard'); await expect(picker).toBeVisible();
    for (const action of ['confirm', 'cancel']) expect((await fetch(`http://localhost:3050/cake-income-adjustments/monthly-review/${action}`, { method: 'PATCH', headers, body: JSON.stringify({ affiliateAccountId: cake.id, settlementMonth: '2026-09' }) })).status).toBe(403);
    ...
    await picker.fill('2025-12'); await picker.press('Enter'); await picker.press('Tab');
    ...
    // July failure fixtures are isolated and never queued for provider execution.
    const july = new Date('2026-07-01');
    for (const [index, reasonCode] of ['SUB_ID_NOT_MAPPED', 'SUB_ID_MISSING'].entries()) {
      const fixture = await db.syncTask.create({ data: { settlementMonth: july, sourceType: 'affiliate_income', taskType: 'affiliate_income', platform: accounts[index].platform, affiliateAccountId: accounts[index].id, status: 'completed', successCount: index ? 4 : 0, failedCount: 1, requestPayload: { settlementMonth: '2026-07' }, finishedAt: new Date() } });
      await db.syncUnmatchedEvent.create({ data: { settlementMonth: july, sourceType: 'affiliate_income', taskType: 'affiliate_income', affiliateAccountId: accounts[index].id, syncTaskId: fixture.id, reasonCode } });
    }
    await db.incomeRecord.create({ data: { employeeId: firstEmployee.id, affiliateAccountId: accounts[1].id, settlementMonth: july, source: 'browser-july', incomeUsd: '123', status: 'confirmed', rawData: { fixture: 'SIMULATED_LOCAL_ONLY' } } });
    const pp = await db.syncTask.create({ data: { settlementMonth: july, sourceType: 'card_spend', taskType: 'photonpay_card', provider: 'photonpay', platform: 'photonpay', status: 'retry_wait', successCount: 1109, failedCount: 1, lastErrorCategory: 'RATE_LIMITED', requestPayload: { settlementMonth: '2026-07' } } });
    for (const [state, expected] of [['retry_wait', '供应商限流，等待自动重试'], ['running', '上次请求被限流，正在重试'], ['failed', '供应商限流，本次任务已结束，需手动重试']]) {
      await db.syncTask.update({ where: { id: pp.id }, data: { status: state } });
      await page.reload();
      await expect(picker).toBeVisible(); await picker.fill('2026-07'); await picker.press('Enter'); await picker.press('Tab');
      const sourceLine = page.locator('.finance-source-line');
      await expect(sourceLine.getByText('未入账', { exact: true })).toHaveCount(1);
      await expect(sourceLine.getByText('1 条 SUB 未映射；成功 0 条 / 失败 1 条', { exact: true })).toBeVisible();
      await expect(sourceLine.getByText('1 条缺少 SUB；成功 4 条 / 失败 1 条', { exact: true })).toBeVisible();
      await expect(sourceLine.getByText(`${expected}；成功 1109 条 / 失败 1 条`, { exact: true })).toBeVisible();
      await expect(sourceLine.getByRole('button', { name: '未匹配事件', exact: true })).toHaveCount(2);
      await expect(sourceLine.getByRole('button', { name: '同步详情', exact: true })).toHaveCount(3);
      if (state !== 'failed') await expect(sourceLine.getByText(state === 'running' ? '刷新中' : '等待自动重试', { exact: true })).toBeVisible();
      else await expect(sourceLine.getByText('等待自动重试', { exact: true })).toHaveCount(0);
      await expect(page.getByText('整月已覆盖', { exact: true })).toHaveCount(0);
      await expect(page.locator('.finance-table tbody tr.ant-table-row').getByText('$123.00', { exact: true })).toHaveCount(3);
      const file = `july-${state}.png`;
      await page.screenshot({ path: path.join(output, file), fullPage: true, animations: 'disabled' }); screenshots.push(file);
    }
    if (errors.length) throw new Error(errors.join('\n'));
    fs.writeFileSync(path.join(output, 'browser-evidence.json'), JSON.stringify({ commit: revision, dirty: Boolean(execFileSync('git', ['status', '--porcelain'], { cwd: root, encoding: 'utf8' }).trim()), buildHashes: { api: treeHash(path.join(root, 'apps/api/dist')), web: treeHash(path.join(root, 'apps/web/dist')) }, screenshots, employees: 8, affiliates: 6, realLocalPostgres: true, simulatedProviders: true, isolatedSchema: true, originalReviewInputsUntouched: true, builtWebApiBase: 'http://localhost:3050', verified: ['CAKE default warning, explicit zero-difference confirmation, cancellation, unchanged refresh retention, native change review, draft/confirmed/disabled adjustments, account-month navigation and readonly denial', 'zero and mixed SUB attribution failures', 'rate-limited retry_wait/running/final failure', 'July old amounts preserved without coverage', 'three viewport core-column bounds', 'collapsed navigation', 'fee values', 'server detail page 2', 'failed refresh and honest coverage', 'three read routes 200/401 and four mutation routes 403'] }, null, 2));
