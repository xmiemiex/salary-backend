...
const output = path.join(root, 'tmp/monthly-finance-rate-limit-fix');
...
(async () => {
  ...
  try {
    ...
    for (const [state, expected] of [['retry_wait', '供应商限流，等待自动重试'], ['running', '上次请求被限流，正在重试'], ['failed', '供应商限流，本次任务已结束，需手动重试']]) {
      ...
      await page.reload();
      await expect(picker).toBeVisible(); await picker.fill('2026-07'); await picker.press('Enter');
    ...
    fs.writeFileSync(path.join(output, 'browser-evidence.json'), JSON.stringify({ commit: revision, dirty: Boolean(execFileSync('git', ['status', '--porcelain'], { cwd: root, encoding: 'utf8' }).trim()), buildHashes: { api: treeHash(path.join(root, 'apps/api/dist')), web: treeHash(path.join(root, 'apps/web/dist')) }, screenshots, employees: 8, affiliates: 6, realLocalPostgres: true, simulatedProviders: true, isolatedSchema: true, originalReviewInputsUntouched: true, builtWebApiBase: 'http://localhost:3050', verified: ['zero and mixed SUB attribution failures', 'rate-limited retry_wait/running/final failure', 'July old amounts preserved without coverage', 'three viewport core-column bounds', 'collapsed navigation', 'fee values', 'server detail page 2', 'failed refresh and honest coverage', 'three read routes 200/401 and four mutation routes 403'] }, null, 2));
