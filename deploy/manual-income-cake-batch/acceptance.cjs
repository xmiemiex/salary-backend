...
(async () => {
  ...
  for (const month of ['2026-07', '2026-08', '2026-09']) {
    ...
    if (mode === 'verify') {
      ...
      for (const row of data.rows.filter(row => !row.attributionPending && row.key !== 'unassigned').slice(0, 2)) {
        const entries = await api('/dashboard/monthly/manual-income?' + new URLSearchParams({ settlementMonth: month, rowKey: row.key }));
        assert(Array.isArray(entries.items)); assert.equal(typeof entries.confirmedTotal, 'string');
      }
