...
(async () => {
  ...
  const result = await db.$transaction(async tx => {
    ...
    if (migrations.length !== 28 || migrations.some(m => !m.finished_at || m.rolled_back_at || expected[m.migration_name] !== m.checksum)) throw Error('MIGRATION_DRIFT');
    ...
    const reviewCount = Number((await tx.$queryRaw`SELECT count(*)::int AS n FROM cake_monthly_income_reviews`)[0].n);
    const reviewRows = await tx.cakeMonthlyIncomeReview.findMany({ orderBy: [{ affiliateAccountId: 'asc' }, { settlementMonth: 'asc' }] });
    ...
    return {
      ...
      preservation: { cakeReviews: hash(reviewRows), allCardSpend: hash(allCardSpend), allSyncTasks: hash(tasks), allIncome: hash(income), manualIncome: hash(manualIncome), manualCost: hash(manual), providerFees: hash(fees), adposFees: hash(adposFees), mappings: hash(mappings), employeeSubIds: hash(employees), locks: hash(locks), providerEmailAliases: hash(aliases), providerCardExclusions: hash(exclusions) },
