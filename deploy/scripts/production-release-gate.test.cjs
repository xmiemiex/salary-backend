...
test('production env check accepts the immutable task98 release format', () => {
  const environment = validProductionEnv();
  environment.RELEASE_IMAGE_TAG = 'task98-0123456789ab';
  const result = spawnSync(process.execPath, [envCheck], { env: environment, encoding: 'utf8' });
  assert.equal(result.status, 0);
  assert.match(result.stdout, /ENV_CHECK name=RELEASE_IMAGE_TAG status=pass/);
});
...
test('production env check accepts the immutable task102 release format', () => {
  const environment = validProductionEnv();
  environment.RELEASE_IMAGE_TAG = 'task102-0123456789ab';
  const result = spawnSync(process.execPath, [envCheck], { env: environment, encoding: 'utf8' });
  assert.equal(result.status, 0);
  assert.match(result.stdout, /ENV_CHECK name=RELEASE_IMAGE_TAG status=pass/);
});
...
test('production env check rejects unsafe or malformed release tags', () => {
  const invalidTags = [
    ...
    'arbitrary',
    '../task98-0123456789ab',
    'task98-0123456789abcdef0123456789abcdef',
    'task98-0123456789a',
    ...
    'task102-0123456789a',
    'task102-0123456789abc',
    'task98-0123456789aG',
  ];
  for (const releaseTag of invalidTags) {
    const environment = validProductionEnv();
    environment.RELEASE_IMAGE_TAG = releaseTag;
    const result = spawnSync(process.execPath, [envCheck], { env: environment, encoding: 'utf8' });
    assert.equal(result.status, 1, `expected RELEASE_IMAGE_TAG=${JSON.stringify(releaseTag)} to fail`);
    assert.match(result.stdout, /ENV_CHECK name=RELEASE_IMAGE_TAG status=fail/);
  }
});
...
test('standard gate refreshes read-only evidence and mounts it into the gate', () => {
  ...
  assert.match(script, /task102-\[0-9a-f\]/);
