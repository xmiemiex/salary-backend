#!/usr/bin/env bash
# CAKE monthly review; one additive evidence-table migration. Default is a read-only plan.
set -Eeuo pipefail
umask 077
mode=${1:-plan}
if [[ "$mode" == plan ]]; then
  echo 'CAKE ea3b9f31e398; 27 to 28 migrations; existing HTTPS origins. Supply STAGE and APPROVED_CHECKS_SHA256, then deploy-approved in the authorized sudo session. No action performed.'
  exit 0
fi
...
[[ "$STAGE" == /home/salaryops/cake-monthly-review-* && "$STAGE" != *..* && "$STAGE" != *$'\n'* ]]
...
commit=ea3b9f31e398a0aa4ea29413f3e42fedd731973a
tag=cake-review-ea3b9f31e398
sha=f1cd08271416929a0abdc08ef2875534beaae4cdccbd89b968d4d249319df466
schema_sha=c0112c4987133722e0b8996786572a5c001e1412bfa84879092a6534bc2c6010
archive="$stage/source-ea3b9f31e398.tar.gz"
release="/opt/salary-settlement-admin/releases/$tag"
evidence="/opt/salary-settlement-admin/release-evidence/$tag"
old=monthly-hotfix-4328cada3fe0
old_api=sha256:a0b62028409b94c22cfdabfad3bc33ff2639030a7aebfd25aada2b8d96dfde0e
old_web=sha256:7361ddee07ca26020b8bbeb878dc494d97d5ca0b7c6fd3509891efe3658c6ca1
...
failed() {
  ...
  if [[ "$phase" == stopped || "$phase" == startup || "$phase" == runtime_checks ]]; then
    ...
    set_tag "$old"
    if compose up -d --no-build api web >"$evidence/rollback.log" 2>&1 && health salary-settlement-admin-api-1 && health salary-settlement-admin-web-1; then status ROLLED_BACK_EXACT_HOTFIX_IMAGES_ADDITIVE_SCHEMA_RETAINED; else status ROLLBACK_NEEDS_ATTENTION; fi
...
phase=stopped
status STOPPING_WRITERS_FOR_ADDITIVE_MIGRATION
...
status APPLYING_SINGLE_APPROVED_EVIDENCE_TABLE_MIGRATION
...
phase=complete
status CAKE_DEPLOYMENT_COMPLETE_READ_ONLY_ACCEPTANCE
export_evidence
