...
(async () => {
  ...
  if (Object.keys(expected).length !== 28 || !expected['20260906010000_transaction_page_resume']) throw Error('CANDIDATE_MIGRATIONS_CHANGED');
  if (expected['20260907010000_cake_monthly_income_review'] !== 'f130f05ce5641d04c41589472302baa4ffecd4f8ed9d5c659fc58ebb3d646aed') throw Error('UNAPPROVED_NEW_MIGRATION');
  const result = await db.$transaction(async tx => {
    ...
    if (migrations.length !== (mode === 'before' ? 27 : 28) || migrations.some(m => !m.finished_at || m.rolled_back_at || expected[m.migration_name] !== m.checksum)) throw Error('MIGRATION_DRIFT');
    ...
    if (active) throw Error('ACTIVE_SYNC');
    if (mode === 'before' && migrations.some(m => m.migration_name === '20260907010000_cake_monthly_income_review')) throw Error('UNEXPECTED_EXISTING_REVIEW_MIGRATION');
    const reviewCount = mode === 'before' ? null : Number((await tx.$queryRaw`SELECT count(*)::int AS n FROM cake_monthly_income_reviews`)[0].n);
    if (reviewCount !== null && reviewCount !== 0) throw Error('UNEXPECTED_REVIEW_WRITES');
    const allCardSpend = await tx.cardSpendEvent.findMany({ orderBy: { id: 'asc' } });
    const tasks = await tx.syncTask.findMany({ orderBy: { id: 'asc' } });
    ...
    return {
      reviewCount, at: new Date().toISOString(), migrations: migrations.map(m => m.migration_name), flagsFalse: true, activeSync: active,
      julyLocked: locks.some(r => r.settlementMonth.toISOString().slice(0, 10) === '2026-07-01' && r.status === 'locked'),
      preservation: { allCardSpend: hash(allCardSpend), allSyncTasks: hash(tasks), allIncome: hash(income), manualIncome: hash(manualIncome), manualCost: hash(manual), providerFees: hash(fees), adposFees: hash(adposFees), mappings: hash(mappings), employeeSubIds: hash(employees), locks: hash(locks), providerEmailAliases: hash(aliases), providerCardExclusions: hash(exclusions) },
