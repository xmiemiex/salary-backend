...
- 共有 13 个有证据的候选项：7 个 P1、5 个 P2、1 个 P3。
...
| B-02 | 可靠性/工程 | 将 task 编号生产入口长期化并归档一次性 rollout | 2 | 3 | 4 | 3 | 3 | P2 |
| B-03 | 可靠性/运维 | Release Gate Evidence、回滚副本和日志保留策略 | 2 | 3 | 4 | 3 | 3 | P2 |
| E-02 | 工程效率 | 收口旧环境变量兼容并统一开发/生产文档 | 2 | 2 | 3 | 2 | 2 | P2 |
| E-03 | 工程效率 | 更新已过期的初始化与架构阶段文档 | 1 | 2 | 2 | 1 | 1 | P2 |
| C-04 | 安全 | 多实例前的共享登录限流方案 | 2 | 1 | 3 | 4 | 3 | P3 |
...
- 证据：34 个 deploy script 中有 12 个 task 编号文件；当前交接仍引用
  `/home/salaryops/task85-post-go-check.sh`，权限 helper 仍硬编码 task84，仓库保留
  `task82`、`task90`、`task93`、`task94` rollout。
- 业务影响：长期值守入口与历史 rollout 混杂，维护者难判断权威脚本和一次性证据脚本。
- 生产/安全风险：中；错误替换或删除可能破坏回滚和审计。
- 依赖：先建立“当前安装入口、历史 rollout、仅证据复现”清单。
- 生产操作：若替换已安装入口则需要；仅仓库归档不需要。
- Migration：不需要。
- RC：不修改 RC。
- 验收：稳定入口使用功能命名；历史脚本只读归档并标注不可复用；hash、安装路径、回滚和
  Runbook 同步；不删除仍被生产引用的文件。

### B-03 Evidence、回滚副本和日志保留策略

- 证据：`docs/operations/production-runbook.md:64` 明确要求清理/归档 release gate
  Evidence；`production-handoff.md:58` 只给出“按月或每次发布后”，未定义保留数量、
  上限、法律保全或删除审批。Task94还保留 root-only 回滚副本。
- 业务影响：无限增长和超龄 Evidence 误用风险持续存在。
- 生产/安全风险：中高；清理过度会损失审计/回滚证据，清理不足会占满磁盘。
- 依赖：Release、Operations、Security/Data owner。
- 生产操作：策略落地和首次清理需要单独窗口；任务95不执行。
- Migration：不需要。
- RC：不影响。
- 验收：按 Evidence 类型定义 owner、保留期、容量上限、legal hold、dry-run、精确目标、
  失败停止和审计；不得触及备份 30 天 retention 或 Accepted 风险。

### E-02 旧环境变量兼容收口

- 证据：`README.md:39` 明确仍兼容 `PORT`、`WEB_ORIGIN`、
...
- 业务影响：配置来源重复，增加环境漂移和排障成本。
- 生产/安全风险：中；直接删除兼容可能导致旧环境启动失败。
- 依赖：先以 env check 证明所有受支持环境已使用新变量，再分阶段告警和删除。
- 生产操作：最终删除兼容时需要常规发布和环境核对。
- Migration：不需要。
- RC：不修改现有 RC。
- 验收：单一变量表、兼容弃用告警、测试覆盖优先级和非法值；生产 env check 全绿后才能
  删除旧分支。

### E-03 更新过期初始化与阶段文档

- 证据：`docs/initialization-plan.md` 仍称不包含 Docker Compose、真实同步、复杂页面和
  结算流程；当前仓库已包含 Compose、真实 adapters、完整业务页和结算服务。
  `deploy/production-architecture.md` 标题和状态仍是任务78的 No-Go 计划，容易与当前
  Full Go 交接混淆。
- 业务影响：新维护者可能按历史边界判断当前能力。
- 生产/安全风险：低。
- 依赖：以当前代码、RC 和生产交接为准，保留历史背景但加显著状态说明。
- 生产操作：不需要。
- Migration：不需要。
- RC：不影响。
- 验收：历史计划与当前事实分栏；所有链接有效；不把未验证生产事实写成完成。

### C-04 多实例前共享登录限流

- 证据：`docs/development.md:90` 明确登录限流仅为单 API 进程内、每 IP 每分钟 5 次；
  多实例需要网关或共享存储。当前生产架构只有一个 API 容器，没有扩容证据。
- 业务影响：仅在多实例或横向扩容前上升。
- 生产/安全风险：中；共享限流错误会误封管理员或放宽攻击面。
- 依赖：扩容决定、网关/共享存储架构和可观测性。
- 生产操作：未来扩容窗口需要。
- Migration：当前不需要。
- RC：不影响。
- 验收：在扩容前完成跨实例一致限流、可信代理 IP、误封恢复和安全测试；当前只记录观察。
...
任务95只创建本规划文档。未修改业务/测试代码、Prisma schema、migration、部署脚本或
systemd；未连接生产、未执行 sudo、未查询生产数据库、未部署/重启/切流、未运行 production
permissions smoke、未刷新 production Evidence、未修改 Git tag。
