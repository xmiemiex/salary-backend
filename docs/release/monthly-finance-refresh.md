...
只有四种正式月度适配器完成全来源查询与所有写入后，才生成 monthlyCoverage 证据。预览、校准、短窗、目标卡子集以及未知请求选项均不给出整月证明。大盘忽略后续无关预览，保留较早正式成功的时间。查询时间与来源覆盖时间分别展示。当月成功为截至时间快照；进入下一GMT+8月份后，各来源必须覆盖至结束不含的下月1日00:00才能认定历史月完整。月中证明保留真实时间和原金额，显示待补刷，不自动延长证明；这不代表后续结算更新永久终止。旧 completed 缺少证明时显示不完整，需要正式重刷验证。
